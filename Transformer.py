...
"""
Transformer model for predicting Hexameter lines
Claude.ai assisted
"""
...
def prepare_data(data, max_length=80):
    ...
    :param data:
    :param max_length:
    :return: x_shape, y_shape, and vocabulary size
    """
...
def build_transformer_model(vocab_size, max_length=80):
...
def load_transformer_model():
    """Load the trained transformer model and vocabulary"""
    print("Loading Transformer model...")

    # Load character mapping
    with open('char_to_id_transformer.json', 'r') as f:
        char_to_id = json.load(f)

    # Determine vocab size from char_to_id
    vocab_size = max(char_to_id.values()) + 1

    # Build model architecture
    model = build_transformer_model(vocab_size, max_length=80)

    # Load weights
    model.load_weights('best_transformer_weights.weights.h5')

    print(f"Transformer model loaded! Vocab size: {vocab_size}")
    return model, char_to_id


def predict_transformer(model, char_to_id, line, max_length=80):
    """
    Predict the scansion pattern for a Latin line using the Transformer model

    Args:
        model: Trained Transformer model
        char_to_id: Character to ID mapping dictionary
        line: Latin text line to scan
        max_length: Maximum sequence length (default 80)

    Returns:
        Pattern string (e.g., "DSDS") or None if error
    """
    try:
        # Prepare input
        line_lower = line.lower()[:max_length]
        encoded = [char_to_id.get(c, 0) for c in line_lower]
        encoded += [0] * (max_length - len(encoded))
        ...
        # Convert to pattern (threshold at 0.5)
...
def train_enhanced_transformer(filename='hexameter_lines.json'):
    """
    Train model
    :param filename:
    :return: model
    ...
    checkpoint = keras.callbacks.ModelCheckpoint(
        'best_transformer_weights.weights.h5',
    ...
    with open('char_to_id_transformer.json', 'w') as f:
...
if __name__ == "__main__":
    model, accuracy, history = train_enhanced_transformer('hexameter_lines.json')